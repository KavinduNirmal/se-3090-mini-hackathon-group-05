...
import { listRestaurants } from '@/lib/server/admin';
...
export default async function AdminRestaurantsPage({ searchParams }: PageProps) {
  ...
  try {
    restaurants = await listRestaurants({ status, q });
  } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Review registrations, verify licenses and manage restaurant access to the platform.
          </p>
        </div>
      </div>
      ...
      {error ? (
        <div className="rounded-2xl border border-border bg-card p-6 text-center shadow-sm">
          <p className="font-bold text-foreground">Could not load restaurants</p>
        ...
        <>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <form action="/admin/restaurants" method="GET" className="flex w-full items-center gap-2 sm:w-auto">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <Input
                  name="q"
                  defaultValue={q}
                  placeholder="Search name, type, city or licence…"
